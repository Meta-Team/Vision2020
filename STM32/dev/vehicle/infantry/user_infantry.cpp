...
// adjustments for vision system to reduce static error, fixed value;
float yaw_adj = 0;
float pitch_adj = 0;

float yaw_vision = 0;
float pitch_vision = 0;
    ...
    while (!shouldTerminate()) {
        ...
        if (!InspectorI::remote_failure() /*&& !InspectorI::chassis_failure()*/ && !InspectorI::gimbal_failure()) {
            if ((Remote::rc.s1 == Remote::S_MIDDLE && Remote::rc.s2 == Remote::S_UP) ||
                ...
                GimbalLG::set_target(yaw_vision, pitch_vision);

            } else if (Remote::rc.s1 == Remote::S_MIDDLE && Remote::rc.s2 == Remote::S_DOWN) {

                /// Vision - Yaw + Pitch
                // enemy_info.yaw_angle and enemy_info.pitch_angle is set by vision system
                // notice there must be some system delay so the target angles may not be real-time;
      
                GimbalLG::set_action(GimbalLG::ABS_ANGLE_MODE);

//                yaw_adj += -Remote::rc.ch0 * (gimbal_rc_yaw_max_speed * USER_THREAD_INTERVAL / 3000.0f);
//                pitch_adj += -Remote::rc.ch1 * gimbal_pitch_max_angle * USER_THREAD_INTERVAL / 3000.0f;
//                float pitch_temp = pitch_adj + VisionPort::enemy_info.pitch_angle;
                // yaw_vision = yaw_vision + (VisionPort::enemy_info.yaw_angle * USER_THREAD_INTERVAL / 1000.0f)
                //         -Remote::rc.ch0 * (gimbal_rc_yaw_max_speed * USER_THREAD_INTERVAL / 1000.0f);
                // // ch0 use right as positive direction, while GimbalLG use CCW (left) as positive direction
                // float temp = pitch_vision + (VisionPort::enemy_info.pitch_angle * USER_THREAD_INTERVAL / 1000.0f);
                // if(temp < -15)
                //     pitch_vision = -15;
                // else if (temp > 15)
                //     pitch_vision = 15;
                ...
                // yaw_vision = VisionPort::enemy_info.yaw_angle * USER_THREAD_INTERVAL / 1000.0f - Remote::rc.ch0 * (gimbal_rc_yaw_max_speed * USER_THREAD_INTERVAL / 1000.0f);
                yaw_vision = -Remote::rc.ch0 * 100 + VisionPort::enemy_info.yaw_angle*4;
                pitch_vision = -Remote::rc.ch1 * 50 +  VisionPort::enemy_info.pitch_angle*2;
                // float pitch_target;
                // if (Remote::rc.ch1 >= 0) pitch_target = Remote::rc.ch1 * gimbal_pitch_max_angle;
                // else yaw_vision = 0;
//                float pitch_temp = VisionPort::enemy_info.pitch_angle;
//                pitch_temp = pitch_temp > gimbal_pitch_max_angle ?  gimbal_pitch_max_angle:pitch_temp;
//                pitch_temp = pitch_temp < gimbal_pitch_min_angle ?  gimbal_pitch_min_angle:pitch_temp;
//
//                if(Remote::rc.ch1 > 0){
//                    VisionPort::enemy_info.pitch_angle = 0;
//                    VisionPort::enemy_info.yaw_angle = 0;
//                }
